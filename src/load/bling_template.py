...
def criar_linha(item, colunas_bling):
    ...
    # ===== MAPA DE COLUNAS =====
    mapa = {
        "Código": item.get("Codigo Produto", ""),
        ...
        "Produto Variação": item.get("Sku", ""),  # coloquei Sku pois costuma ser o ID da variação
        "Tipo Produção": "Variação",
        "Código Pai": item.get("Sku", ""),
