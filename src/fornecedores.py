        ...
        if arquivo.lower().endswith(".xml"):
            ...
            try:
                ...
                if emitente is not None:
                    fornecedor = {
                        "CNPJ": emitente.findtext(
                            "nfe:CNPJ", default="", namespaces=ns
                        ),
                        "RazaoSocial": emitente.findtext(
                            "nfe:xNome", default="", namespaces=ns
                        ),
                        "NomeFantasia": emitente.findtext(
                            "nfe:xFant", default="", namespaces=ns
                        ),
                        "Logradouro": emitente.findtext(
                            "nfe:enderEmit/nfe:xLgr", default="", namespaces=ns
                        ),
                        "Numero": emitente.findtext(
                            "nfe:enderEmit/nfe:nro", default="", namespaces=ns
                        ),
                        "Bairro": emitente.findtext(
                            "nfe:enderEmit/nfe:xBairro", default="", namespaces=ns
                        ),
                        "Municipio": emitente.findtext(
                            "nfe:enderEmit/nfe:xMun", default="", namespaces=ns
                        ),
                        "UF": emitente.findtext(
                            "nfe:enderEmit/nfe:UF", default="", namespaces=ns
                        ),
                        "CEP": emitente.findtext(
                            "nfe:enderEmit/nfe:CEP", default="", namespaces=ns
                        ),
                        "Telefone": emitente.findtext(
                            "nfe:enderEmit/nfe:fone", default="", namespaces=ns
                        ),
                    ...
                    if not any(
                        f["RazaoSocial"] == fornecedor["RazaoSocial"]
                        for f in fornecedores
                    ):
                        fornecedores.append(fornecedor)

            except ET.ParseError:
                logger.info(f"Erro ao ler o XML: {arquivo}")
            except Exception as e:
                logger.info(f"Erro inesperado em {arquivo}: {e}")
