...
def escolher_prioritario(grupo):
    ...
    return grupo[grupo['Fornecedor'].isin(preferidos)].head(1) if preferidos else grupo.head(1)
...
def parte_comum(strings, sensibilidade=0.7):
    ...
    contagem = Counter({p: sum(fuzz.ratio(p, t)/100 >= sensibilidade for tokens in token_lists for t in tokens)
                        for p in todas_palavras})
    limite = max(1, int(len(strings)*0.8))
    comuns = [w for w, c in contagem.items() if c >= limite]
    base = " ".join([t for t in token_lists[0] if any(fuzz.ratio(t, w)/100 >= sensibilidade for w in comuns)])
...
print("✅ Arquivo final.xlsx gerado com sucesso!")
