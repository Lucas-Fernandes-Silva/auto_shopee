...
from dados.dados import marca_variacoes, marcas_adicionais
from dados.env import fornecedores_web_scraping
...
def escolher_prioritario(grupo: pd.DataFrame) -> pd.DataFrame:
...
def detectar_marca(desc: str) -> str | float:
...
def similaridade_media(strings: list[str]) -> float:
    pares = list(itertools.combinations(strings, 2))
    if not pares:
        return 1.0
    return float(np.mean([fuzz.token_set_ratio(a, b) / 100 for a, b in pares]))

def parte_comum(strings: list[str], sensibilidade: float = 0.7) -> str:
    token_lists = [normalizar(s).split() for s in strings if s]
    if not token_lists:
        return ""
    todas_palavras = {p for tokens in token_lists for p in tokens if not re.match(r"^\d+[Xx/]\d+$", p)}
    contagem = Counter(
        {p: sum(fuzz.ratio(p, t) / 100 >= sensibilidade for tokens in token_lists for t in tokens)
         for p in todas_palavras}
    )
    limite = max(1, int(len(strings) * 0.8))
    comuns = [w for w, c in contagem.items() if c >= limite]
    base = " ".join(
        [t for t in token_lists[0] if any(fuzz.ratio(t, w) / 100 >= sensibilidade for w in comuns)]
    )
    return re.sub(r"\b\d+([Xx/]\d+)?\b", "", base).strip()

# ---------------- INICIALIZAÇÃO ---------------- #

df: pd.DataFrame = df_enriquecido.copy()
...
marcas: Set[str] = set(map(limpar_texto, marcas_adicionais))
marcas_planilha: Set[str] = set(map(limpar_texto, df["Marca"].dropna().astype(str)))
...
df["Marca"] = df["Marca"].fillna(df["Descrição"].apply(detectar_marca)).fillna("GENÉRICO")
...
def preencher_categoria(desc: str) -> str:
    match = process.extractOne(desc, com_categoria["Descricao_norm"], scorer=fuzz.token_sort_ratio)
    if not match:
        return "Sem categoria semelhante"
    nome_similar, score, idx = match
    return com_categoria.iloc[idx]["Categoria"] if score > 40 else "Sem categoria semelhante"
...
# ---------------- CHAVE PARA SIMILARIDADE ---------------- #

df["Chave"] = (df["Descrição"].apply(normalizar) + " " + df["Categoria"].apply(normalizar)).str.strip()

# ---------------- AGRUPAMENTO DE VARIAÇÕES VETORIZADO ---------------- #

# Garante colunas para resultado
for col in ["Tipo", "SKU_Pai", "ID_Variacao"]:
    if col not in df.columns:
        df[col] = pd.Series(dtype=object)

strings = df["Chave"].tolist()

# Cria matriz de distância 1 - token_sort_ratio
tqdm.pandas(desc="Calculando similaridades")
matriz = process.cdist(strings, strings, scorer=fuzz.token_sort_ratio, score_cutoff=0)
dist = 1 - np.array(matriz) / 100.0

# Clustering hierárquico
linkage_matrix = linkage(dist, method="average")
df["ID_Variacao"] = fcluster(linkage_matrix, t=(100 - 90) / 100, criterion="distance")

# SKU PAI = primeiro SKU do grupo
sku_col = "Sku" if "Sku" in df.columns else "Código"
df["SKU_Pai"] = df.groupby("ID_Variacao")[sku_col].transform("first")

# Tipo PAI/FILHO
df["Tipo"] = np.where(df[sku_col] == df["SKU_Pai"], "PAI", "FILHO")

# ---------------- BASE E VARIANTE ---------------- #

df["Base"], df["Variante"] = "", ""
for gid, grupo in tqdm(df.groupby("ID_Variacao", group_keys=False), desc="Gerando bases"):
    ...
    df.loc[grupo.index, "Variante"] = grupo["Chave"].apply(lambda x: x.replace(comum, "").strip())
