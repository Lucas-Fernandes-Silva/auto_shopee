...
class CategoryFiller:
    ...
    def _buscar_categoria(self, descricao):
        ...
        _, score, idx = match
        return self.com_categoria.iloc[idx]['Categoria'] if score > 40 else "Sem categoria semelhante"

    def aplicar(self):
        if self.sem_categoria.empty:
            return self.df
        self.sem_categoria['Categoria'] = self.sem_categoria['Descrição'].map(self._buscar_categoria)
