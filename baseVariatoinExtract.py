...
class BaseVariantExtractor:
    ...
    def parte_comum(strings, sensibilidade=0.7):
        ...
        if not token_lists:
            return ""
        todas = {p for tokens in token_lists for p in tokens if not re.match(r"^\d+[Xx/]\d+$", p)}
    ...
    def aplicar(self, df):
        ...
        for gid, grupo in tqdm(df.groupby("ID_Variacao", group_keys=False), desc="Gerando bases"):
            ...
            comum = self.parte_comum(chaves, sens) or Normalizer.normalize(grupo["Descrição"].iloc[0])
            df.loc[grupo.index, "Base"] = comum
            df.loc[grupo.index, "Variante"] = grupo["Chave"].map(lambda x: x.replace(comum, "").strip())
        return df
